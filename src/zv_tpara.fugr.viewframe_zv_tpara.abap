*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZV_TPARA
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZV_TPARA            .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZV_TPARA'
         EXPORTING
              FCODE          = READ
...
EXTRACT = ZV_TPARA_EXTRACT
TOTAL = ZV_TPARA_TOTAL
              X_HEADER       = X_HEADER
              X_NAMTAB       = X_NAMTAB
...
CALL FUNCTION 'VIEWPROC_ZV_TPARA'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZV_TPARA-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZV_TPARA_EXTRACT
TOTAL = ZV_TPARA_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZV_TPARA-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZV_TPARA'
...
TOTAL = ZV_TPARA_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZV_TPARA-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZV_TPARA-UPD_FLAG NE SPACE.

...
*&---------------------------
*&Changed by/date      : 05.05.2023
*&DESCRIPTION          : change script to smartform and changed mail sending part as well.
*&Request No.          :BCDK932884, BCDK932890, BCDK932912,BCDK932920
*&--------------------------------------------------------------------------------------*
*&Changed by/date      : 05.05.2023
*&DESCRIPTION          : for mail sendding - delete blank territory record to avoid
*&                       receiver exception.
*&Request No.          :BCDK933042
*&--------------------------------------------------------------------------------------*
*&Changed by/date      : 01.07.2024
*&DESCRIPTION          : Corr.for ZM name at header.
*&Request No.          :BCDK935393,BCDK935395
*&---------------------------------------------------------------------------------------*
*&Changed by/date      : 02.08.2024
*&DESCRIPTION          : Corr.to remove wrong div.data in output.
*&                       select curr.Yr data from ZPQV/ZCUMPSO.
*&Request No.          :BCDK935563,BCDK935565
*&---------------------------------------------------------------------------------------*
...
data : it_zdsmter21 type table of zdsmter,
...
*select-options : ter for zdsmter-bzirk  matchcode object ZSR7 no intervals.
...
*PERFORM wrtGRPDATA.
  ...
  if r5 = 'X'.
    loop at it_tabfin into wa_tabfin.
      read table it_tac11 into wa_tac11 with key rm = wa_tabfin-bzirk.
      if sy-subrc = 0.
        wa_tam1-pernr = wa_tac11-pernr.
    ...
    delete adjacent duplicates from it_tam1 comparing pernr.
  endif.
  ...
  if r1 = 'X' .
    ...
    perform dispdata.
  elseif r2 = 'X' .
******  FORMNM = 'ZSR12C_FRM'.        "'ZSR12C_FRM'
******  PERFORM OPEN_FORM.
******  PERFORM FRM_SR12_PRN.
******  PERFORM CLOSE_FORM1.
...
form nameupdt .
  ...
  select * from zdsmter into table it_zdsmter where zmonth eq mth and zyear eq yr and bzirk in zone.
  loop at it_zdsmter into wa_zdsmter where bzirk+0(2) cs 'D-'.
    ...
    if sy-subrc = 0.
      ...
      wa_tac11-pernr = mpernr.
      wa_tac11-ename = mename.
      wa_tac11-hq = mhq.
      collect wa_tac11 into it_tac11.
    endif.
    clear wa_tac11.
  endloop.
  clear : it_zdsmter , wa_zdsmter.
  clear : mplan.
  select * from zdsmter into table it_zdsmter where zmonth eq mth and zyear eq yr and zdsm in zone.
...
*        IF YTERRALLC-endda >= CRYRDT.
  ...
  delete adjacent duplicates from it_tac2 .
  sort it_tac2 by reg rm bzirk.
  loop at it_tac2 into wa_tac2.
*   WRITE :/1 wa_TAC2-REG , wa_TAC2-RM,  WA_tac2-bzirk.
    select single * from zdrphq where bzirk eq wa_tac2-reg.
    mename = ''.
    mhq = ''.
    mpernr = 0.

    select single * from yterrallc where bzirk eq wa_tac2-reg and begda le mdate and endda ge mdate.
    if sy-subrc = 0.
      mplan = yterrallc-plans.
    else.
      clear mplan.
    ...
    clear wa_tac3.
*     WRITE :/1 wa_TAC2-REG , wa_TAC2-RM,  WA_tac2-bzirk, MENAME .
  endloop.
endform.                    " NAMEUPDT
...
*  WRITE :/1 'APR-14'.
    ...
    select bzirk  month01 month02 month03 month04 month05 month06
                    month07 month08 month09 month10 month11 month12
                      from ysd_dist_targt into table it_tgtp1
                          for all entries in it_tac3
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp2
          for all entries in it_tac3
          where  zmonth = mth and zyear = yr  and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp2
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp3
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp3
                     for all entries in it_tac3  where  zmonth = mth and
                        zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    ...
    select bzirk  netval from zrcumpso into table it_netvalp4
                 for all entries in it_tac3  where  zmonth = mth and
                    zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp5
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp5
                     for all entries in it_tac3  where  zmonth = mth and
                        zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp6
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp6
                 for all entries in it_tac3  where  zmonth = mth and
                    zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp7
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp7
                   for all entries in it_tac3  where  zmonth = mth and
                      zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp8
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp8
                   for all entries in it_tac3  where  zmonth = mth and
                      zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp9
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp9
                     for all entries in it_tac3  where  zmonth = mth and
                        zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp10
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp10
                     for all entries in it_tac3  where  zmonth = mth and
                        zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp11
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp11
                     for all entries in it_tac3  where  zmonth = mth and
                        zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
  call function 'RE_ADD_MONTH_TO_DATE'
  ...
  if it_tac3 is not initial.
    select bzirk matnr grosspts grossqty  from zrpqv into table it_tabp12
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp12
                     for all entries in it_tac3  where  zmonth = mth and
                        zyear  = yr and bzirk = it_tac3-bzirk.
    select bzirk  netval from zrcumpso into table it_netvalp12
              for all entries in it_tac3  where  zmonth = mth and
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab1
    ...
    select bzirk  month01 month02 month03 month04 month05 month06
                  month07 month08 month09 month10 month11 month12
                    from ysd_dist_targt into table it_tgt1
                        for all entries in it_tac3
                          where  trgyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval1
    select bzirk  netval from zcumpso into table it_netval1
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab2
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab2
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval2
    select bzirk  netval from zcumpso into table it_netval2
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab3
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab3
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval3
    select bzirk  netval from zcumpso into table it_netval3
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab4
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab4
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval4
    select bzirk  netval from zcumpso into table it_netval4
                      for all entries in it_tac3  where  zmonth = mth and
                         zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab5
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab5
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval5
    select bzirk  netval from zcumpso into table it_netval5
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab6
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab6
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval6
    select bzirk  netval from zcumpso into table it_netval6
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab7
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab7
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval7
    select bzirk  netval from zcumpso into table it_netval7
                      for all entries in it_tac3  where  zmonth = mth and
                         zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab8
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab8
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval8
    select bzirk  netval from zcumpso into table it_netval8
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab9
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab9
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval9
    select bzirk  netval from zcumpso into table it_netval9
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab10
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab10
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval10
    select bzirk  netval from zcumpso into table it_netval10
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk  netval from zrcumpso into table it_netval11
    select bzirk  netval from zcumpso into table it_netval11
                   for all entries in it_tac3  where  zmonth = mth and
                      zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
*****    select bzirk matnr grosspts grossqty  from zrpqv into table it_tab12
    select bzirk matnr grosspts grossqty  from zpqv into table it_tab12
      for all entries in it_tac3
      where  zmonth = mth and zyear = yr and bzirk = it_tac3-bzirk.
*****    select bzirk  netval from zrcumpso into table it_netval12
    select bzirk  netval from zcumpso into table it_netval12
                        for all entries in it_tac3  where  zmonth = mth and
                           zyear  = yr and bzirk = it_tac3-bzirk.
  endif.
...
form dispdata .
  ...
  layout-window_titlebar  = 'SR-12 : ZM/MONTH WISE PRODUCT GROUPWISE REPORT'.
...
form top.
  ...
  wa_comment-typ = 'A'.
  if r1 = 'X'.
    wa_comment-info = 'SR-12 : ZM/MONTH WISE PRODUCT GROUPWISE VALUE SALE AS OF'.
  elseif r2 = 'X'.
    wa_comment-info = 'SR-12 : ZM/MONTH WISE PRODUCT GROUPWISE UNIT SALE AS OF'.
  endif.
  c_date := mdate.
...
form grpdata .
  loop at it_tabtot into wa_tabtot.
    read table it_tac3 into  wa_tac3 with key reg = wa_tabtot-bzirk.
...
*    WRITE : /1 wa_TABFIN-bzirk , wa_TABFIN-GRP_CODE, WA_ZMTPT_ACH-TOTALSE_ACH, WA_ZMTPT_ACH-TARGET.
    ...
    elseif wa_tabfin-spart = '70'.
      mtotpso = wa_ztotpso-ls - wa_ztotpso-hbe.
    else.
...
include zsr12n_open_formf01.
*INCLUDE ZSR12_OPEN_FORMF01.
*&---------------------------------------------------------------------*
*&      Form  WRTGRPDATA
*&---------------------------------------------------------------------*
*       text
...
include zsr12n_sr12emailf01.
*INCLUDE ZSR13_SR12EMAILF01.

include zsr121n_valuedataf01.
*&---------------------------------------------------------------------*
*&      Form  GET_DATA_SFORM
*&---------------------------------------------------------------------*
*       text
...
form get_data_sform .
  ...
  sort it_tabfin by bzirk spart prn_seq .
  ctr = 0.
  loop at it_tabfin into wa_tabfin.
    mtitle = 'SR-12 : ZM MONTH/PRODUCT GROUPWISE'.
...
*****    PERFORM write_main_form.
    ...
    tag = 'P.Y.YPM'.
*****    elname = 'CDET'.
    avgqty = wa_tabfin-pavg_ypm..
    qty1 = wa_tabfin-papr_ypm.
    qty2 = wa_tabfin-pmay_ypm.
    qty3 = wa_tabfin-pjun_ypm.
    qty4 = wa_tabfin-pjul_ypm.
    qty5 = wa_tabfin-paug_ypm.
    qty6 = wa_tabfin-psep_ypm.
    qty7 = wa_tabfin-poct_ypm.
    qty8 = wa_tabfin-pnov_ypm.
    qty9 = wa_tabfin-pdec_ypm.
    qty10 = wa_tabfin-pjan_ypm.
    qty11 = wa_tabfin-pfeb_ypm.
    qty12 = wa_tabfin-pmar_ypm.
    qtysum =  ''.
*****    PERFORM write_main_form.
...
*****      PERFORM group-total.
*****      PERFORM summwrite.
***** commented above perform and added below for new smartform layout
...
*****    ENDAT.
  ...
  if gt_fdata[] is not initial and ( r3 = 'X' or r5 = 'X') .
    clear gt_fdata_all[].
...
*****  delete gt_fdata_zm WHERE pernr is INITIAL.
...
*****  delete gt_fdata_zm WHERE pernr is INITIAL.
  ...
  delete gt_fdata_zm_all where bzirk is initial.
****  form printing
  clear fm_name.
  call function 'SSF_FUNCTION_MODULE_NAME'
    exporting
...
****Add the email body content to document
      ...
      concatenate 'SR-12:MONTHWISE PRODUCT GROUP SALE TREND - DSM WISE' wa_d1 'TO ' wa_d2 gv_text into gv_subject separated by space.
...
****      attachment name
      clear lv_att_sub.
      concatenate 'SR-12' gw_fdata_zm_all-ename into lv_att_sub separated by '.'.

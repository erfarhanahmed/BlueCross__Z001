...
*  WRITE :/1 MYR , MM.
  ...
  loop at it_zexpdata into wa_zexpdata.
    wa_tab1-matnr = wa_zexpdata-matnr.
...
*    write : /1 wa_zexpdata-zyear, wa_zexpdata-zmonth, m_date.
    if m_date < s_budat-high and m_date >=  c_date.
      wa_tabf1-matnr = wa_zexpdata-matnr.
...
*  WRITE :/1 MYR , MM, C_DATE .
  ...
  loop at it_zexpbudget into wa_zexpbudget.
    ...
    if m_date < s_budat-high and m_date >=  c_date.
      wa_tab_bf1-matnr = wa_zexpbudget-matnr.
...
form merge_data .
  ...
  loop at it_mara into wa_mara.
    read table it_tab1 into wa_tab1 with key matnr = wa_mara-matnr.
    if sy-subrc eq 0.
      wa_tabtot-curr_qty = wa_tab1-qty.
      wa_tabtot-curr_val = wa_tab1-val.
    endif.
    read table it_tab_b1 into wa_tab_b1 with key matnr = wa_mara-matnr.
    if sy-subrc eq 0.
      wa_tabtot-curr_bqty = wa_tab_b1-qty.
      wa_tabtot-curr_bval = wa_tab_b1-val.
    endif.
    read table it_tabf1 into wa_tabf1 with key matnr = wa_mara-matnr.
    if sy-subrc eq 0.
    ...
    read table it_tab_bf1 into wa_tab_bf1 with key matnr = wa_mara-matnr.
    if sy-subrc eq 0.
    ...
    if wa_tabtot-curr_qty <> 0 or wa_tabtot-curr_val <> 0 or
      ...
      select single * from zprdgroup where prd_code  eq wa_mara-matnr and rep_type = 'EXP'.
      if sy-subrc eq 0.
        wa_tabtot-grp_code = zprdgroup-grp_code.
        wa_tabtot-bezei    = zprdgroup-grp_name.
        wa_tabtot-prn_seq  = zprdgroup-prn_seq.
      endif.
...
form top.
  ...
  wa_comment-typ = 'A'.
  wa_comment-info = 'SR-2-B : EXPORT SALES (GROSS) GROUPWISE SUMMARY '.
  c_date := s_budat.
...
*     wa_tabtot-CURR_BVAL = wa_tab_b1-VAL.
    ...
    if qty1 > 0 or qty2 > 0 or qty3 > 0 or qty4 > 0.
      wa_tabtot-curr_qty = qty1.
      wa_tabtot-curr_val = qty2.
      wa_tabtot-curr_bqty = qty3.
      wa_tabtot-curr_bval = qty4.
      write : /1 wa_mara-matnr , qty1, qty2, qty3, qty4.
      select single * from zprdgroup where prd_code  eq wa_mara-matnr and rep_type = 'EXP'.
      if sy-subrc eq 0.
        wa_tabtot-grp_code = zprdgroup-grp_code.
        wa_tabtot-bezei    = zprdgroup-grp_name.
...
*     pack1 = WA_TABTOT-pack.
    qty1 = wa_tabtot-curr_qty.
    qty2 = wa_tabtot-curr_bqty.
    qty3 = wa_tabtot-curr_val.
    qty4 = wa_tabtot-curr_bval.
    qty5 = wa_tabtot-curr_tqty.
    qty6 = wa_tabtot-curr_tbqty.
    qty7 = wa_tabtot-curr_tval.
    qty8 = wa_tabtot-curr_tbval.
    ...
    mdatael = 'DET1'.
    if wa_tabtot-curr_tval > 0 or wa_tabtot-curr_tbval > 0.
      perform write_line.
    endif.
...
*     concatenate 'ES-4: MONTHLY ACTUAL UNITS SOLD FOR PERIOD' wa_d1 'TO ' wa_d2  into doc_chng-obj_descr separated by space.
  concatenate 'SR-2-B : EXPORT SALES (GROSS)' mtitle+0(6) '&CUM'  into doc_chng-obj_descr separated by space.
...
*      concatenate 'ACTUAL UNIT  FOR PERIOD' wa_d1 'TO ' wa_d2 '.' into objpack-obj_descr separated by space.
  concatenate 'SR-2-B' '' into objpack-obj_descr.    "  separated by space.

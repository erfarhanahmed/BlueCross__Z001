*&---------------------------------------------------------------------*
*& Report ZSAMP_PROCESS_NEW_NEW_A1
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZSAMP_PROCESS_NEW_NEW_A1.
...
DATA : IT_ZDSMTER      TYPE TABLE OF ZDSMTER,
       ...
       IT_PA0302a      TYPE TABLE OF PA0302,
       WA_PA0302a      TYPE PA0302,
...
*    WRITE : / 'QQ',WA_TAB4-REG,WA_TAB4-ZM,WA_TAB4-RM,WA_TAB4-BZIRK.
*  ENDLOOP.
...
*  BREAK-POINT .
  LOOP AT IT_TAG1 INTO WA_TAG1 WHERE SAMP_PRD GE '000000000000800000'.
    IF R2 EQ 'X'.
*      BREAK-POINT.
...
********* START - added for matnr len chng 15.11.2025
    CLEAR lv_matnr.
    lv_matnr = WA_TAG1-SAMP_PRD.
...
* Implement suitable error handling here
    ENDIF.
********* END - added for matnr len chng 15.11.2025
    SELECT SINGLE * FROM MARA WHERE MATNR EQ lv_matnr."WA_TAG1-SAMP_PRD.
    IF SY-SUBRC EQ 0.
      IF MARA-SPART EQ '50'.
        DIV = 'BC'.
...
*        IF SY-SUBRC EQ 0.
      ...
      IF SY-SUBRC EQ 0.
        ...
        ENDIF.
      ENDIF.
*        wa_ZM1-rm = wa_tas2-rm.
*        wa_ZM1-bzirk = wa_tas2-bzirk.
...
*        IF SY-SUBRC EQ 0.
      ...
      IF SY-SUBRC EQ 0.
        ...
        ENDIF.
      ENDIF.
*        wa_REG1-zm = wa_tas2-zm.
*        wa_REG1-rm = wa_tas2-rm.
*        wa_REG1-bzirk = wa_tas2-bzirk.
...
*  clear : <fs_tas1>.
***  DELETE IT_PSO1 WHERE PERNR EQ '00000000'.  Commented on 8.7.24
***  DELETE IT_RM1 WHERE PERNR EQ '00000000'.
***  DELETE IT_ZM1 WHERE PERNR EQ '00000000'.
***  DELETE IT_REG1 WHERE PERNR EQ '00000000'.
*  LOOP AT IT_PSO1 ASSIGNING <FS_TAS1>.
*    WRITE : / 'PSO',<FS_TAS1>-BZIRK,<FS_TAS1>-NETVAL,<FS_TAS1>-DIV.
*  ENDLOOP.
*
*  LOOP AT IT_RM1 ASSIGNING <FS_TAS1>.
*    WRITE : / 'RM',<FS_TAS1>-BZIRK,<FS_TAS1>-NETVAL,<FS_TAS1>-DIV.
*  ENDLOOP.
*
*  LOOP AT IT_ZM1 ASSIGNING <FS_TAS1>.
*    WRITE : / 'ZM',<FS_TAS1>-BZIRK,<FS_TAS1>-NETVAL,<FS_TAS1>-DIV.
*  ENDLOOP.
*
*  LOOP AT IT_REG1 ASSIGNING <FS_TAS1>.
*    WRITE : / 'REG',<FS_TAS1>-BZIRK,<FS_TAS1>-NETVAL,<FS_TAS1>-DIV.
*  ENDLOOP.
...
*  WRITE : / 'MONTH & YEAR',DATE1-LOW,1DATE1,2DATE1,M1,Y1,M2,Y2,M3,Y3.

*  ASSIGN it_zrpqv to <ITAB>.
...
*  ULINE.
  ...
  CLEAR : it_pa0302a ,wa_pa0302a.
  if it_pa0001 IS NOT INITIAL.
    SELECT * FROM PA0302 INTO TABLE it_pa0302a FOR ALL ENTRIES IN it_pa0001 WHERE PERNR EQ it_PA0001-PERNR AND MASSN EQ '01'.
  endif.
...
*                ENDIF.
                READ TABLE it_PA0302a INTO wa_pa0302a with KEY PERNR = WA_PA0001-PERNR  MASSN = '01'.  "7.3.25
                IF SY-SUBRC EQ 0.
                  WA_PPSO1-DOJ = wa_PA0302a-BEGDA.
                ENDIF.
              ENDIF.
            ENDIF.
...
*                ENDIF.
                read TABLE it_PA0302a INTO wa_pa0302a with KEY PERNR = WA_PA0001-PERNR MASSN = '01'.
                IF SY-SUBRC EQ 0.
                  WA_PZM1-DOJ = wa_PA0302-BEGDA.
                ENDIF.
              ENDIF.
...
*                ENDIF.
                read TABLE it_PA0302 INTO wa_pa0302a  with KEY PERNR = WA_PA0001-PERNR MASSN = '01'.
                IF SY-SUBRC EQ 0.
                  WA_PREG1-DOJ = wa_PA0302-BEGDA.
                ENDIF.
              ENDIF.

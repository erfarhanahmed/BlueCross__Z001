...
data : it_tab    type table of itab,
       ...
       wa_sf1    type sf1,
       it_sf2    type table of zsr1,
       wa_sf2    type zsr1.

data : it_tab18_nep type table of itab18,
       wa_tab18_nep type itab18.
...
*            to_dt type sy-datum.
parameters : mon(2) type c,
             yr(4)  type c.

parameter : net radiobutton group r2,
...
*  ELSEIF R7 EQ 'X'.
*    PERFORM NET_EMAIL.
  elseif r22 = abap_true.
    perform net_sale_nep.
*****    perform net_alv.
    perform net_sale_dom.

    if it_tab18[] is not initial and it_tab18_nep[] is not initial.
      perform disp_dom_nep_alv.
    endif.
...
*      CONCATENATE 'SR9 for the period' ltx '-' INTO doc_chng-obj_descr SEPARATED BY space.
  concatenate 'SR-1 GROSS FROM:' wa_d1 'TO ' wa_d2  into doc_chng-obj_descr separated by space.
  ...
  concatenate 'SR-1 GROSS' '.' into objpack-obj_descr separated by space.
...
*
  ...
  layout-window_titlebar  = 'SR-1 : COMPANY SALES (NET-GL) - PRODUCTWISE'..
...
*****  endloop.
  ...
  wa_fieldcat-fieldname = 'MAKTX'.
  wa_fieldcat-seltext_l = 'GROUP4-DESCRIPTION'.
  append wa_fieldcat to fieldcat.
...
*
  ...
  layout-zebra = 'X'.
  layout-colwidth_optimize = 'X'.
  layout-window_titlebar  = 'SR-1 : COMPANY SALES (NET) - PRODUCTWISE'..
...
*
  wa_fieldcat-fieldname = 'MAKTX'.
  wa_fieldcat-seltext_l = 'GROUP4-PRODUCT DESCRIPTION'.
  append wa_fieldcat to fieldcat.
...
*
  ...
  layout-zebra = 'X'.
  layout-colwidth_optimize = 'X'.
  layout-window_titlebar  = 'SR-1 : COMPANY SALES (GROSS) - PRODUCTWISE'..
...
form top.
  ...
  wa_comment-typ = 'A'.
  wa_comment-info = 'SR-1 : COMPANY SALES (GROSS) PRODUCTWISE'.
...
form top1.
  ...
  wa_comment-typ = 'A'.
  wa_comment-info = 'SR-1 : COMPANY SALES (NET) PRODUCTWISE'.
*  WA_COMMENT-INFO = P_FRMDT.
  append wa_comment to comment.
...
form sfform .
  if r7 eq 'X'.
    if net eq 'X'.
      concatenate 'SR-1 : COMPANY SALES (NET) - PRODUCTWISE' month1 '& CUM.' into headtxt1 separated by space.
    elseif gross eq 'X'.
      concatenate 'SR-1 : COMPANY SALES (GROSS) - PRODUCTWISE' month1 '& CUM.' into headtxt1 separated by space.
    endif.
  elseif r10 eq 'X'.
    if net eq 'X'.
      concatenate 'SR-1 : COMPANY SALES (NET) - PRODUCTWISE' month1 '& CUM.' into headtxt1 separated by space.
...
*    concatenate 'SR-1 A (NET) ' year1 '-' m2 'PRODUCT PACK SALE' into headtxt1 separated by space.
  ...
  sort it_tab18 by spart prn_seq grp_code.
  loop at it_tab18 into wa_tab18.
*    if r12 eq 'X'.
*      if ( wa_tab18-grp_code = 53 ) or ( wa_tab18-grp_code = 45 ).
*        if wa_tab18-net_sale ne 0 .
...
**************************************
    ...
    clear : ngrq.
    if wa_sf2-net_sale cs '-'.
      ngrq = wa_sf2-net_sale.
      replace '-' with space into ngrq.
      concatenate '-' ngrq into ngrq.
      condense ngrq no-gaps.
      wa_sf2-net_sale = ngrq.
    endif.
    ...
    clear : ngrq.
    if wa_sf2-cnet_qty cs '-'.
      ngrq = wa_sf2-cnet_qty.
      replace '-' with space into ngrq.
      concatenate '-' ngrq into ngrq.
      condense ngrq no-gaps.
      wa_sf2-cnet_qty = ngrq.
    endif.
    ...
    clear : ngrq.
    if wa_sf2-cnet_sale cs '-'.
      ngrq = wa_sf2-cnet_sale.
      replace '-' with space into ngrq.
      concatenate '-' ngrq into ngrq.
      condense ngrq no-gaps.
      wa_sf2-cnet_sale = ngrq.
    endif.
...
*******************************
    ...
    if z1 eq 'X'.
      if wa_sf2-net_sale ne 0 or wa_sf2-b_pts ne 0.
...
******************************
      ...
      wa_sf2-cnet_sale,wa_sf2-cb_pts,wa_sf2-ach2,wa_sf2-grq,wa_sf2-grv.
      if z1 eq 'X'.
        if wa_sf2-net_qty ne 0 or wa_sf2-b_qty ne 0.
          append wa_sf2 to it_sf2.
        endif.
      else.
...
*************DIVISION TOT*************
    ...
    clear : grp_name.
    if wa_sf1-spart = 50.
      grp_name = 'BLUECROSS DIVISION'.
    elseif wa_sf1-spart = 60.
      grp_name = 'EXCEL DIVISION'.
    elseif wa_sf1-spart eq '70'.
      grp_name = 'BCLS DIVISION'.
    endif.

    at end of spart.
      concatenate grp_name ': TOTAL' into wa_sf2-maktx separated by space.
      wa_sf2-bezei = space.
...
****************************************
    ...
    at last.
      concatenate 'COMPANY TOTAL' ':' into wa_sf2-maktx separated by space.
      wa_sf2-bezei = space.
...
form send_mail  using    p_in_mailid.
  ...
  body = 'Please find the attached SR1 REPORT in PDF format.'.
...
*l_size = l_lines * 255.
  try.
      ...
      i_attachment_subject = 'SR1 REPORT'
...
form net_sale_nep .
  ...
  r13 = abap_true.   "for taking nepal sales only

  clear it_zsales_tab1[].
  select * from zsales_tab1 into table it_zsales_tab1 where datab ge from_dt and datbi le to_dt.
*****  if r13 ne 'X'.
*****    select * from zcredit_tab1 into table it_zcredit_tab1 where datab ge from_dt and datbi le to_dt.
*****    select * from zdebit_tab1 into table it_zdebit_tab1 where datab ge from_dt and datbi le to_dt.
...
form disp_dom_nep_alv .
  ...
  data : lt_tab18     type table of itab18,
         lw_tab18     type itab18,
         lt_tab18_nep type table of itab18,
         lw_tab18_nep type itab18.

  data : lv_cnet_sale  type p,
         lv_lcnet_sale type p,
         lv_net_sale   type p.

  sort it_tab18[] by spart matnr.
  sort it_tab18_nep[] by spart matnr.

  if it_tab18[] is not initial.
    clear lt_tab18[].
    lt_tab18[] = it_tab18[].
    clear it_tab18[].
    ...
    clear lw_tab18.
    clear wa_tab18.
    loop at lt_tab18 into lw_tab18.
      wa_tab18-spart = lw_tab18-spart.
      wa_tab18-net_sale = lw_tab18-net_sale.
      wa_tab18-cnet_sale = lw_tab18-cnet_sale.
      wa_tab18-lcnet_sale = lw_tab18-lcnet_sale.

      lv_cnet_sale = lv_cnet_sale + lw_tab18-cnet_sale.
      lv_lcnet_sale = lv_lcnet_sale + lw_tab18-lcnet_sale.
      at end of spart.
        if lv_lcnet_sale ne 0.
          clear grv.
          grv = ( lv_cnet_sale / lv_lcnet_sale ) * 100 - 100.
*****          grv = ( wa_tab18-cnet_sale / wa_tab18-lcnet_sale ) * 100 - 100.
        endif.
        wa_tab18-grv = grv.
        clear: lv_cnet_sale, lv_lcnet_sale.
      endat.

      collect wa_tab18 into it_tab18.
      clear wa_tab18.
    endloop.
    write:/ 'INDIA - SR-1 : COMPANY SALES (NET) - DIV-WISE'.
    write:/.
*****    write:/ '------------------------------------------------------------------------------------------------------------------'.
    write:/1 'DIV', 10 'VALUE SALE THIS MONTH', 40 'VALUE SALE THIS YTD', 70 'L.YR CUMM SALE' , 90 'VALUE GR THIS YTD.'.
*****    write:/ '------------------------------------------------------------------------------------------------------------------'.
    clear: lv_net_sale,lv_cnet_sale, lv_lcnet_sale.
    loop at it_tab18 into wa_tab18.
      lv_cnet_sale = lv_cnet_sale + wa_tab18-cnet_sale.
      lv_lcnet_sale = lv_lcnet_sale + wa_tab18-lcnet_sale.
      lv_net_sale = lv_net_sale + wa_tab18-net_sale.
      write :/1 wa_tab18-spart , 10 wa_tab18-net_sale , 40 wa_tab18-cnet_sale , 70 wa_tab18-lcnet_sale , 90 wa_tab18-grv.
    endloop.
    ...
    write:/ '------------------------------------------------------------------------------------------------------------------'.
    write :/1 'DOM' , 10 lv_net_sale , 40 lv_cnet_sale , 70 lv_lcnet_sale , 90 grv.
    write:/ '------------------------------------------------------------------------------------------------------------------'.
...
*****      exporting
*****        i_callback_program      = g_repid
*****        i_grid_title            = 'SR-1 : COMPANY SALES (NET) - PRODUCTWISE'
******       I_CALLBACK_PF_STATUS_SET          = ' '
*****        i_callback_user_command = 'USER_COMM'
*****        i_callback_top_of_page  = 'TOP1'
*****        is_layout               = layout
...
*****    endif.
  ...
  if it_tab18_nep[] is not initial.
    clear lt_tab18_nep[].
    lt_tab18_nep[] = it_tab18_nep[].
    clear it_tab18_nep[].

    delete lt_tab18_nep[] where spart = '70'.
    clear lw_tab18_nep.
    clear wa_tab18_nep.
    clear: lv_cnet_sale, lv_lcnet_sale.
    loop at lt_tab18_nep into lw_tab18_nep.
      wa_tab18_nep-spart = lw_tab18_nep-spart.
      wa_tab18_nep-net_sale = lw_tab18_nep-net_sale.
      wa_tab18_nep-cnet_sale = lw_tab18_nep-cnet_sale.
      wa_tab18_nep-lcnet_sale = lw_tab18_nep-lcnet_sale.

      lv_cnet_sale = lv_cnet_sale + lw_tab18_nep-cnet_sale.
      lv_lcnet_sale = lv_lcnet_sale + lw_tab18_nep-lcnet_sale.

      wa_tab18_nep-kondm = 'NP'.
      at end of spart.
        if lv_lcnet_sale ne 0.
          clear grv.
          grv = ( lv_cnet_sale / lv_lcnet_sale ) * 100 - 100.
*****          grv = ( wa_tab18_nep-cnet_sale / wa_tab18_nep-lcnet_sale ) * 100 - 100.
        endif.
        wa_tab18_nep-grv = grv.
        clear: lv_cnet_sale, lv_lcnet_sale.
      endat.

      collect wa_tab18_nep into it_tab18_nep.
      clear wa_tab18_nep.
    endloop.
    ...
    write:/.
    write:/.
    write:/ 'NEPAL - SR-1 : COMPANY SALES (NET) - DIV-WISE'.
    write:/.
*****    write:/ '------------------------------------------------------------------------------------------------------------------'.
    write:/1 'DIV', 10 'VALUE SALE THIS MONTH', 40 'VALUE SALE THIS YTD', 70 'L.YR CUMM SALE' , 90 'VALUE GR THIS YTD.'.
*****    write:/ '------------------------------------------------------------------------------------------------------------------'.
    clear: lv_net_sale,lv_cnet_sale, lv_lcnet_sale.
    loop at it_tab18_nep into wa_tab18_nep.
      lv_cnet_sale = lv_cnet_sale + wa_tab18_nep-cnet_sale.
      lv_lcnet_sale = lv_lcnet_sale + wa_tab18_nep-lcnet_sale.
      lv_net_sale = lv_net_sale + wa_tab18_nep-net_sale.
      write :/1 wa_tab18_nep-spart ,10 wa_tab18_nep-net_sale ,40 wa_tab18_nep-cnet_sale, 70 wa_tab18_nep-lcnet_sale, 90 wa_tab18_nep-grv.
    endloop.
    ...
    write:/ '------------------------------------------------------------------------------------------------------------------'.
    write :/1 'NEP' , 10 lv_net_sale , 40 lv_cnet_sale , 70 lv_lcnet_sale , 90 grv.
    write:/ '------------------------------------------------------------------------------------------------------------------'.
...
*****    layout-zebra = 'X'.
*****    layout-colwidth_optimize = 'X'.
*****    layout-window_titlebar  = 'SR-1 : COMPANY SALES (NET) - PRODUCTWISE FOR NEPAL'..
*****
*****
*****    call function 'REUSE_ALV_GRID_DISPLAY'
*****      exporting
*****        i_callback_program      = g_repid
*****        i_grid_title            = 'SR-1 : COMPANY SALES (NET) - PRODUCTWISE FOR NEPAL'
******       I_CALLBACK_PF_STATUS_SET          = ' '
*****        i_callback_user_command = 'USER_COMM'
*****        i_callback_top_of_page  = 'TOP1'
*****        is_layout               = layout
...
form net_sale_dom .
  ...
  clear : it_zsales_tab1[],it_zcredit_tab1[],it_zdebit_tab1[],it_zbudget_tab1[].
  select * from zsales_tab1 into table it_zsales_tab1 where datab ge from_dt and datbi le to_dt.
  select * from zcredit_tab1 into table it_zcredit_tab1 where datab ge from_dt and datbi le to_dt.
  select * from zdebit_tab1 into table it_zdebit_tab1 where datab ge from_dt and datbi le to_dt.
  select * from zbudget_tab1 into table it_zbudget_tab1 where datab ge from_dt and datbi le to_dt and tag eq 'N'.
  ...
  loop at it_zsales_tab1 into wa_zsales_tab1.
    ...
    pts = wa_zsales_tab1-net + wa_zsales_tab1-ed - wa_zsales_tab1-nep_pts.
    wa_tab11-matnr = wa_zsales_tab1-matnr.
    wa_tab11-pts = pts.
    wa_tab11-qty = wa_zsales_tab1-c_qty.
    wa_tab11-f_qty = wa_zsales_tab1-nep_pts. "wa_zsales_tab1-f_qty.
    wa_tab-matnr = wa_zsales_tab1-matnr.
    collect wa_tab into it_tab.
    clear wa_tab.
    collect wa_tab11 into it_tab11.
  ...
  loop at it_tab into wa_tab.
    ...
    if sy-subrc eq 0.
      ...
      wa_tab15-c_pts = wa_tab12-pts.
      wa_tab15-c_qty = wa_tab12-qty.
      wa_tab15-s_pts = wa_tab15-s_pts - wa_tab15-c_pts.
    endif.
    read table it_tab13 into wa_tab13 with key matnr = wa_tab-matnr.  "dn
    if sy-subrc eq 0.
      wa_tab15-matnr = wa_tab13-matnr.
      wa_tab15-d_pts = wa_tab13-pts.
      wa_tab15-d_qty = wa_tab13-qty.
      wa_tab15-s_pts = wa_tab15-s_pts + wa_tab15-d_pts.
    endif.
    read table it_tab14 into wa_tab14 with key matnr = wa_tab-matnr.  "budget
    if sy-subrc eq 0.
      wa_tab15-matnr = wa_tab14-matnr.
...
*  write : / 'cummulative date',c_date.
  ...
  if it_zsales_tab2 is not initial.
    ...
    loop at it_zsales_tab2 into wa_zsales_tab2.
      wa_ctab1-matnr = wa_zsales_tab2-matnr.
      wa_ctab1-pts = wa_zsales_tab2-net + wa_zsales_tab2-ed - wa_zsales_tab2-nep_pts.
      wa_ctab1-qty = wa_zsales_tab2-c_qty.
      wa_ctab1-f_qty = wa_zsales_tab2-f_qty.
      collect wa_ctab1 into it_ctab1.
      clear wa_ctab1.
...
*      WRITE : / 'budget',c_date,TO_DT,wa_zBUDGET_tab2-matnr,wa_zBUDGET_tab2-qty.
  ...
  loop at it_ta into wa_ta.
    ...
    if sy-subrc eq 0.
      ...
      wa_ctab5-c_pts = wa_ctab2-pts.
      wa_ctab5-c_qty = wa_ctab2-qty.
      wa_ctab5-s_pts = wa_ctab5-s_pts - wa_ctab5-c_pts.
    endif.
    read table it_ctab3 into wa_ctab3 with key matnr = wa_ta-matnr.
    if sy-subrc eq 0.
      wa_ctab5-matnr = wa_ctab3-matnr.
      wa_ctab5-d_pts = wa_ctab3-pts.
      wa_ctab5-d_qty = wa_ctab3-qty.
      wa_ctab5-s_pts = wa_ctab5-s_pts + wa_ctab5-d_pts.
    endif.
    read table it_ctab4 into wa_ctab4 with key matnr = wa_ta-matnr.
    if sy-subrc eq 0.
      wa_ctab5-matnr = wa_ctab4-matnr.
...
*************************************************************************
  ...
  if it_zsales_tab3 is not initial.
    ...
    loop at it_zsales_tab3 into wa_zsales_tab3.
      wa_lctab1-matnr = wa_zsales_tab3-matnr.
      wa_lctab1-pts = wa_zsales_tab3-net + wa_zsales_tab3-ed - wa_zsales_tab3-nep_pts.
      wa_lctab1-qty = wa_zsales_tab3-c_qty.
      wa_lctab1-f_qty = wa_zsales_tab3-f_qty.
      collect wa_lctab1 into it_lctab1.
      clear wa_lctab1.
  ...
  loop at it_t into wa_t.
    ...
    if sy-subrc eq 0.
      ...
      wa_lctab4-c_pts = wa_lctab2-pts.
      wa_lctab4-c_qty = wa_lctab2-qty.
      wa_lctab4-s_pts = wa_lctab4-s_pts - wa_lctab4-c_pts .
    endif.
    read table it_lctab3 into wa_lctab3 with key matnr = wa_t-matnr.
    if sy-subrc eq 0.
      wa_lctab4-matnr = wa_lctab3-matnr.
      wa_lctab4-d_pts = wa_lctab3-pts.
      wa_lctab4-d_qty = wa_lctab3-qty.
      wa_lctab4-s_pts = wa_lctab4-s_pts + wa_lctab4-d_pts .
    endif.
    collect wa_lctab4 into it_lctab4.
    clear wa_lctab4.
  endloop.
